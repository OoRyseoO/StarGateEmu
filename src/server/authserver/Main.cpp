...
extern int main(int argc, char **argv)
{
    ...
    sLog->outString(" ");
    sLog->outString(" ");
    sLog->outString("  Project StarGateEmu 2011(c) Open-sourced Game Emulation ");
    sLog->outString(" ");
    ...
    {
        sLog->outError("StarGateAuth can not bind to %s:%d", bind_ip.c_str(), rmport);
        return 1;
    }
...
#ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outString("StarGateAuth process priority class set to HIGH");
            else
